...
int instructionblock_counter = 0;
...
int main(void) { 
    ...
    //Temporary enabling GPIO for IO-board for debugging
    DDRD = 0xFF;
    PORTD= 0x00;
    DDRC = 0xF0;
    PORTC = 0x3F;
    ...
    while(phase == upload) {
        ...
        if((interrupt_count) >= 32) { // Checking for successful upload
            ...
            file_processing(&cakefile, instructionblock, instructionblock_counter); // Proccessing the recieved array
            instructionblock_counter++;
            if(instruction_count == ceil(((float)filesize/32.0))){
                PORTB |= (1<<PB5);
                LCD_init();
                LCD_set_cursor(0,0);
