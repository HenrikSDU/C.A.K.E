...
int file_processing(CAKEFILE* cakefile, volatile char* instructionblock, int savelocation){
    ...
    for(int read_index = 0; read_index < 32; read_index++){
        ...
        if(instructionblock[read_index] == 'G'){
            
            cakefile->instruction_locations[savelocation] = 1; //a one indicates that at the position a instruction is saved

            cakefile->path[savelocation].extruder_inst = (instructionblock[read_index + 1] & 0x0F) - 1;//extracts the instruction  -- atoi() -
        ...
        if(instructionblock[read_index] == 'X'){//check for the beginning of a coordinate block - they all are in the form of X(coordinate in ASCII)Y(coordinate in ASCII)
            ...
            while(instructionblock[read_index + string_scanner] != 'Y'){
                
                cakefile->path[savelocation].table_coord.x *= 10;//shifting the previous number one decimal to the side
                cakefile->path[savelocation].table_coord.x += (instructionblock[read_index + string_scanner] & 0x0F);//adding the new number
            ...
            while((instructionblock[read_index + string_scanner] & 0b00110000)){//checking whether there a numbers hidden in ASCII - the expression comes from the ASCII characteristics - numbers are saved like: 0b0011xxxx with the x's being the actual number; new line characters are 0b0000xxxx 
                
                cakefile->path[savelocation].table_coord.y *= 10;//shifting the previous number one decimal to the side
                cakefile->path[savelocation].table_coord.y += (instructionblock[read_index + string_scanner] & 0x0F);//adding the new number
